...
app.get("/api/stats", async (req, res) => {
  try {
    ...
    const ram = Number(((ramUsed / ramTotal) * 100).toFixed(1));
    ...
    res.json({
      ...
      cpuTemp: temp.main || 0,
      net: {
        rx: netStats[0] ? Number((netStats[0].rx_sec / 1024).toFixed(1)) : 0,
        tx: netStats[0] ? Number((netStats[0].tx_sec / 1024).toFixed(1)) : 0,
        iface: netStats[0] ? netStats[0].iface : "unknown",
      },
    });
  } catch (err) {
    console.error("ERROR /api/stats:", err);
    res.status(500).json({
      error: "Gagal membaca stats",
      detail: err.message
    });
  }
});
